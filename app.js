...
function renderMatches(){
  ...
  box.innerHTML=matches.map(m=>{
    ...
    if(locked){
      return `<div class="match"><div><strong>${m.home}</strong><small>${m.group}<br>${date}</small>${resultText}</div><div class="locked">${T[currentLang].closed}</div><div><strong>${m.away}</strong></div></div>`;
    }
    return `<div class="match" data-match="${m.id}">
      <div><strong>${m.home}</strong><small>${m.group}<br>${date}</small>${resultText}</div>
      <div class="score">
...
function renderAdminResults(){
  ...
  box.innerHTML = matches.map(m=>{
    ...
    return `<div class="admin-match">
      <div><strong>${m.home}</strong><small>${m.group}<br>${m.start}</small></div>
      <div class="score">
...
window.saveResultsAndCalculate = async function(){
  ...
  try{
    ...
    console.error(e);
    status.textContent = "❌ Error al guardar resultados.";
  }
};
