...
function renderMatches(){
  ...
  box.innerHTML=matches.map(m=>{
    ...
    if(locked){
      return `<div class="match"><div><strong>${m.home}</strong><small>${m.group}<br>${date}</small></div><div class="locked">${T[currentLang].closed}</div><div><strong>${m.away}</strong></div></div>`;
    }
    return `<div class="match" data-match="${m.id}">
      <div><strong>${m.home}</strong><small>${m.group}<br>${date}</small></div>
      <div class="score">
